...
function handleSignupSubmit(event) {
  ...
  formInputs.forEach(input => {
    ...
    if (input.type === "text" && input.value === "") {
      ...
      if (input.name === "email") {
        errorMessage.insertAdjacentHTML(
          "beforeend",
          `<div class="ui pointing label">Please enter an email address. </div>`
        );
      } else {
        errorMessage.insertAdjacentHTML(
          "beforeend",
          `<p>Please enter a ${input.name}. </p>`
    ...
    if (input.name === "password" && input.value.length < 5 && !emptyField) {
      ...
      errorMessage.insertAdjacentHTML(
        "beforeend",
        `<p>Your password must be at least 5 characters long. </p>`
      );
    }
    ...
    if (
      ...
      !isTextFormatCorrect(input.value) &&
      !emptyField
    ) {
      formIsValid = false;
      // add red border to input
      ...
      errorMessage.insertAdjacentHTML(
        "beforeend",
        `<p>Your ${input.name} cannot contain any special characters. </p>`
      );
    }
    ...
    if (
      input.name === "email" &&
      !isEmailFormatCorrect(input.value) &&
      !emptyField
    ) {
      formIsValid = false;
      // add red border to input
